 ...
 * @author R
 */
public class mod_luogo extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        Template t = c.getTemplate("mod_luogo.ftl.html");
        ...
        if(request.getSession(false)!=null){
            ...
            AttrezzaturaDataLayer atdl=dl.getAttrezaturaDataLayer(dl.getC());
            LuogoDataLayer ldl=dl.getLuogoDataLayer(dl.getC());
            Luogo ltmp=ldl.readLuogobyid(idluogo);
            List<Attrezzatura> tmp=atdl.readAttrezzatura();
            
            data.put("nomeluogo",ltmp.getNome_luogo());
            data.put("tipoluogo",ltmp.getTipo_luogo());
            data.put("indirizzo",ltmp.getIndirizzo());
            data.put("posti",ltmp.getPosti());
            data.put("idluogo",idluogo);
            
            atdl.modListAtt(tmp,idluogo);
            tmp.get(0).isIspresent();
            int sizelist=tmp.size();
            data.put("attrezzatura",tmp);
            data.put("attsize",sizelist);
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            ...
            Logger.getLogger(add_luogo.class.getName()).log(Level.SEVERE, null, ex);
